...
class MfwLostSpider(BaseSpider):
    name = "mfw-lost"
    allowed_domains = ["mafengwo.cn"]
    prefix_url = "http://www.mafengwo.cn"
    lost_ids = []
    start_urls = [
        "http://www.mafengwo.cn/group/info.php?iid=663235&start=100&ownerid=3945295",
        "http://www.mafengwo.cn/group/info.php?iid=601420&start=1800&ownerid=482339"
    ]
    
    # for index in lost_ids:
    #     url = "http://www.mafengwo.cn/i/%s.html" % index
    ...
    def get_next_link(self, response, owner_str):
        ...
        if len(page_links) > 0:
            ...
            if link.select('text()').extract()[0] == u'\xa0Next\xa0>>':
                ...
                return self.make_requests_from_url(self.prefix_url+url+owner_str)
            else:
